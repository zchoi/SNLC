...
# Semantic cross-entropy (SCE) loss.
class SCE(nn.Module):
    ...
    def forward(self, prediction: torch.Tensor, label: torch.Tensor, topk: int = 1) -> torch.Tensor:
        ...
        Embedding_sim_masked = Embedding_sim.masked_fill(diagoal_mask, -2.) # num_cls num_cls

        values, indices = torch.topk(Embedding_sim_masked, 5) # num_cls 5

        loss_sum = 0.

        for i in range(topk):
            mapping_dic = indices[:, i].view(-1,1) # num_cls 1
            ...
            loss_sum += self.__instance_bce_with_logits(prediction, semantic_a)
